...
public abstract class HuaweiMobileServicesUtil {
    ...
    public static int isHuaweiMobileServicesAvailable(Context context, int i) {
        ...
        HMSLog.m86966i("HuaweiMobileServicesUtil", "hmsPackageName is " + hMSPackageName);
        ...
        if (PackageManagerHelper.PackageStates.NOT_INSTALLED.equals(packageStates)) {
            HMSLog.m86966i("HuaweiMobileServicesUtil", "HMS is not installed");
            return 1;
        } else if (PackageManagerHelper.PackageStates.DISABLED.equals(packageStates)) {
            HMSLog.m86966i("HuaweiMobileServicesUtil", "HMS is disabled");
            return 3;
        } else {
            ...
            HMSLog.m86966i("HuaweiMobileServicesUtil", "connect versionCode:" + hmsVersionCode);
            if (!HMSPackageManager.getInstance(context).isApkUpdateNecessary(i)) {
                return 0;
            }
            HMSLog.m86966i("HuaweiMobileServicesUtil", "The current version does not meet the minimum version requirements");
            return 2;
        }
    }
