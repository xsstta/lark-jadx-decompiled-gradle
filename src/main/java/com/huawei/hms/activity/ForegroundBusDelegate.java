...
public class ForegroundBusDelegate implements IBridgeActivityDelegate {
    ...
    private void checkMinVersion() {
        if (getActivity() == null) {
            HMSLog.m86964e(TAG, "checkMinVersion failed, activity must not be null.");
            ...
            HMSLog.m86964e(TAG, "checkMinVersion failed, and no available lib exists.");
        } else {
            startApkHubActivity();
        }
    }
    ...
    public void startApkHubActivity() {
        HMSLog.m86966i(TAG, "startApkHubActivity");
        Activity activity = getActivity();
        if (activity == null) {
            HMSLog.m86964e(TAG, "startApkHubActivity but activity is null");
            return;
        }
        ...
        try {
            ...
            HMSLog.m86965e(TAG, "Launch sign in Intent failed. hms is probably being updated：", e);
    ...
    public class MyAvailableCallBack implements AvailableAdapter.AvailableCallBack {
        ...
        public void onComplete(int i) {
            ...
            HMSLog.m86966i(ForegroundBusDelegate.TAG, "version check failed");
    ...
    private void succeedReturn(int i, Intent intent) {
        HMSLog.m86966i(TAG, "succeedReturn");
        Activity activity = getActivity();
        if (activity != null) {
    ...
    private static void resolution(Activity activity, AvailableAdapter availableAdapter, AvailableAdapter.AvailableCallBack availableCallBack) {
        if (activity == null) {
            HMSLog.m86966i(TAG, "null activity, could not start resolution intent");
        }
