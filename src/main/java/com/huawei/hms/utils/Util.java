...
public class Util {
    ...
    public static int getHmsVersion(Context context) {
        ...
        if (packageManager == null) {
            HMSLog.m86964e(TAG, "In getHmsVersion, Failed to get 'PackageManager' instance.");
            return 0;
        }
        try {
            ...
            HMSLog.m86966i(TAG, "In getHmsVersion, Failed to read meta data for the HMS VERSION.");
            return 0;
        } catch (PackageManager.NameNotFoundException unused) {
            HMSLog.m86964e(TAG, "In getHmsVersion, Failed to read meta data for the HMS VERSION.");
            return 0;
        }
    }
    ...
    public static String getServiceActionMetadata(Context context) {
        ...
        try {
            ...
            HMSLog.m86966i(TAG, "In getServiceActionMetadata, Failed to read meta data for the SERVICE ACTION.");
        } catch (PackageManager.NameNotFoundException unused) {
            HMSLog.m86967w(TAG, "In getServiceActionMetadata, Failed to read meta data for the SERVICE ACTION.");
        }
        serviceAction = "";
        return "";
    }
    ...
    public static boolean isAvailableLibExist(Context context) {
        ...
        synchronized (LOCK_OBJECT) {
            if (!availableInit) {
                ...
                PackageManager packageManager = context.getPackageManager();
                if (packageManager == null) {
                    HMSLog.m86964e(TAG, "In isAvailableLibExist, Failed to get 'PackageManager' instance.");
                    try {
                        Class.forName("com.huawei.hms.adapter.AvailableAdapter");
                    } catch (ClassNotFoundException unused) {
                        HMSLog.m86964e(TAG, "In isAvailableLibExist, Failed to find class AvailableAdapter.");
                    }
                } else {
                    try {
                        ApplicationInfo applicationInfo = packageManager.getPackageInfo(context.getPackageName(), SmActions.ACTION_ONTHECALL_EXIT).applicationInfo;
                        if (!(applicationInfo == null || applicationInfo.metaData == null || (obj = applicationInfo.metaData.get(AVAILABLE_LOADED)) == null || !String.valueOf(obj).equalsIgnoreCase("yes"))) {
                            HMSLog.m86966i(TAG, "available exist: true");
                        }
                    } catch (PackageManager.NameNotFoundException unused2) {
                        HMSLog.m86964e(TAG, "In isAvailableLibExist, Failed to read meta data for the availableLoaded.");
                    }
        ...
        HMSLog.m86966i(TAG, "available exist: " + isAvailableAvailable);
    ...
    public static String getAppName(Context context, String str) {
        if (context == null) {
            HMSLog.m86964e(TAG, "In getAppName, context is null.");
            return "";
        }
        PackageManager packageManager = context.getPackageManager();
        if (packageManager == null) {
            HMSLog.m86964e(TAG, "In getAppName, Failed to get 'PackageManager' instance.");
            return "";
        }
        try {
            ...
            HMSLog.m86964e(TAG, "In getAppName, Failed to get app name.");
            return "";
        }
    }
    ...
    public static void unBindServiceCatchException(Context context, ServiceConnection serviceConnection) {
        try {
            HMSLog.m86966i(TAG, "Trying to unbind service from " + serviceConnection);
            context.unbindService(serviceConnection);
        } catch (Exception e) {
            HMSLog.m86964e(TAG, "On unBindServiceException:" + e.getMessage());
        }
    }
