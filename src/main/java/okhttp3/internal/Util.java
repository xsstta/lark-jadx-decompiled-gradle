...
public final class Util {
    ...
    public static final Comparator<String> NATURAL_ORDER = new Comparator<String>() {
        /* class okhttp3.internal.Util.C696631 */
    ...
    private static String inet6AddressToAscii(byte[] bArr) {
        ...
        while (i < bArr.length) {
            if (i == i2) {
                ...
                cVar.mo244550g((long) (((bArr[i] & 255) << 8) | (bArr[i + 1] & 255)));
                i += 2;
            }
        }
    ...
    public static ThreadFactory threadFactory(final String str, final boolean z) {
        return new ThreadFactory() {
            /* class okhttp3.internal.Util.ThreadFactoryC696642 */
            ...
            public Thread newThread(Runnable runnable) {
                Thread new_insert_after_java_lang_Thread_by_knot = new_insert_after_java_lang_Thread_by_knot(new Thread(runnable, str));
                new_insert_after_java_lang_Thread_by_knot.setDaemon(z);
    ...
    public static Charset bomAwareCharset(AbstractC69699e eVar, Charset charset) throws IOException {
        ByteString byteString = UTF_8_BOM;
        if (eVar.rangeEquals(0, byteString)) {
            eVar.skip((long) byteString.size());
            return UTF_8;
        }
        ByteString byteString2 = UTF_16_BE_BOM;
        if (eVar.rangeEquals(0, byteString2)) {
            eVar.skip((long) byteString2.size());
            return UTF_16_BE;
        }
        ByteString byteString3 = UTF_16_LE_BOM;
        if (eVar.rangeEquals(0, byteString3)) {
            eVar.skip((long) byteString3.size());
            return UTF_16_LE;
        }
        ByteString byteString4 = UTF_32_BE_BOM;
        if (eVar.rangeEquals(0, byteString4)) {
            eVar.skip((long) byteString4.size());
            return UTF_32_BE;
        }
        ByteString byteString5 = UTF_32_LE_BOM;
        if (!eVar.rangeEquals(0, byteString5)) {
            return charset;
        }
        eVar.skip((long) byteString5.size());
        return UTF_32_LE;
    }
    ...
    public static boolean discard(AbstractC69723u uVar, int i, TimeUnit timeUnit) {
        try {
    ...
    public static boolean skipAll(AbstractC69723u uVar, int i, TimeUnit timeUnit) throws IOException {
        long j;
        long nanoTime = System.nanoTime();
        if (uVar.timeout().hasDeadline()) {
            j = uVar.timeout().deadlineNanoTime() - nanoTime;
        } else {
            j = Long.MAX_VALUE;
        }
        uVar.timeout().deadlineNanoTime(Math.min(j, timeUnit.toNanos((long) i)) + nanoTime);
        try {
            C69694c cVar = new C69694c();
            while (uVar.read(cVar, 8192) != -1) {
                cVar.mo244551g();
            }
            if (j == Long.MAX_VALUE) {
                uVar.timeout().clearDeadline();
            } else {
                uVar.timeout().deadlineNanoTime(nanoTime + j);
            }
            return true;
